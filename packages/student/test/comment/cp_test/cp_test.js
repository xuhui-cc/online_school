...
Page({
  ...
  back_btn:function(e){
    ...
    if(type == 1){
      if(that.data.isshare == 1){
        wx.switchTab({
          url: app.getPagePath('index') + '?gid=' + that.data.gid,
        })
      }else{
        wx.navigateBack({
          delta: 1  // 返回上一级页面。
  ...
  update_cpsubmit:function(){
    ...
    app.ols.update_cpsubmit(params).then(d => {
      ...
      if (d.data.code == 0) {
        ...
        wx.redirectTo({
          url: app.getPagePath('cp_report') + '?mid=' + that.data.mid,
        })
      } else {
    ...
    if (d.data.code == 0) {
      console.log(d.data.data,"分享判断接口数据")
         if(d.data.data.status == 2){
          wx.redirectTo({
            url: app.getPagePath('cp_report') + '?mid=' + d.data.data.mid
          })
  ...
  onShareAppMessage: function () {
    ...
    return app.shareTool.getShareReturnInfo('0,1', 'cp_test', params, this.shareImagePath ? this.shareImagePath : '', wx.getStorageSync('shareHead').paper + that.data.ques_info.title)
  },

...
Page({
    ...
    app.ols.group_detail3(params).then(d => {
      ...
      for(var j=0;j<d.data.data.member.length;j++){
        if(d.data.data.member[j].avatar.indexOf("/") == 0){
          ...
          d.data.data.member[j].avatar = app.globalData.dummy + d.data.data.member[j].avatar
          //表示strCode是以ssss开头；
  ...
  onShareAppMessage: function () {
    ...
    return app.shareTool.getShareReturnInfo('0,1', '/pages/group_share/group_share', paramsStr, '../../images/share1.png', '领军网校')
  }
})